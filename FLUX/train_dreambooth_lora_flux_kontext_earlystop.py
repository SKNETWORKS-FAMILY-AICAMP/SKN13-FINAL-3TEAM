...
from transformers import CLIPTokenizer, PretrainedConfig, T5TokenizerFast, CLIPProcessor, CLIPModel
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                # ---- Early Stopping 추가 ----
                val_image = images[0].convert("RGB")
                clip_score = compute_clip_score(val_image, args.validation_prompt)
                logger.info(f"[Epoch {epoch}] CLIP score: {clip_score:.4f}")
                ...
                if clip_score > best_clip:
                    ...
                    counter += 1
                    if counter >= patience:
                        logger.info("⏹ Early stopping triggered (no CLIP improvement)")
                        accelerator.end_training()   # 학습 종료
                        break
                if not args.train_text_encoder:
