...
function PrototypeLab() {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      .then(response => {
        console.log('[PrototypeLab.jsx] ✅ sendChatMessage .then() 진입');
        console.log('[PrototypeLab.jsx] ✅ 스트리밍 완료 - 최종 응답:', {
          streamingMessageId,
          finalResponse: response.response?.substring(0, 100) + (response.response?.length > 100 ? '...' : ''),
          responseLength: response.response?.length,
          timestamp: new Date().toISOString()
        });
        console.log('[PrototypeLab.jsx] ✅ 전체 response:', response);
        if (response.isJson && response.response && response.response.trim()) {
        ...
        if (response.completionStatus && response.checklistData) {
          const backendChecklistData = response.checklistData;
          // console.log('[PrototypeLab.jsx] 백엔드 체크리스트 데이터:', backendChecklistData);
          // console.log('[PrototypeLab.jsx] 기존 체크리스트 데이터:', checklistData);
          setChecklistData(prev => {
            const updated = {
              ...prev,
              ...backendChecklistData
            };
            // console.log('[PrototypeLab.jsx] 업데이트된 체크리스트 데이터:', updated);
            return updated;
          });
        }
        
        // 생성된 결과들 처리
        console.log('[PrototypeLab.jsx] 🔍 generatedResults:', response.generatedResults);
        if (response.generatedResults) {
          response.generatedResults.forEach(result => {
            const resultId = `result-${Date.now()}-${result.result_id || Math.random()}`;
            
            // 마지막 메시지가 result 타입이면 추가하지 않음
            setMessages(prev => {
              const lastMessage = prev[prev.length - 1];
              
              if (lastMessage && lastMessage.type === 'result') {
                console.log('[PrototypeLab.jsx] ⚠️ 마지막 메시지가 result 타입이므로 추가하지 않음');
                return prev; // 마지막 메시지가 result 타입이면 추가하지 않음
              }
              ...
              console.log('[PrototypeLab.jsx] ✅ 새로운 결과 메시지 추가:', resultMessage);
              return [...prev, resultMessage];
            });
          });
        }
        
        // response를 다음 .then()으로 전달
        return response;
      })
      .then(response => {
        // auto_retry 플래그가 있으면 자동으로 파이프라인 재실행
        console.log('[PrototypeLab.jsx] 🔍 전체 response:', response);
        
        // response가 존재하는지 확인
        if (!response) {
          console.log('[PrototypeLab.jsx] ⚠️ response가 undefined입니다.');
        }
        
        console.log('[PrototypeLab.jsx] 🔍 응답 디버깅:', { auto_retry: response.auto_retry, generation_type: response.generation_type });
        
        if (response.auto_retry) {
          console.log('[PrototypeLab.jsx] 🔄 auto_retry 감지 - 파이프라인 자동 재실행');
          
          // 1초 후 자동으로 같은 메시지로 파이프라인 재실행 (무한 루프 방지)
          setTimeout(async () => {
            console.log('[PrototypeLab.jsx] 🔄 파이프라인 자동 재실행 시작');
            try {
              // sendChatMessage 직접 호출
              ...
              console.log('[PrototypeLab.jsx] 🔄 재실행 완료:', retryResponse);
              
              // retryResponse 처리 (jsonMessage 추가)
              if (retryResponse.isJson && retryResponse.response && retryResponse.response.trim()) {
                const jsonMessage = { id: `json-${Date.now()}`, type: 'ai', content: retryResponse.response, timestamp: new Date().toISOString(), isStreaming: false };
                setMessages(prev => [...prev, jsonMessage]);
              }
              
              // retryResponse 처리 (체크리스트 데이터 업데이트)
              if (retryResponse.completionStatus && retryResponse.checklistData) {
                const backendChecklistData = retryResponse.checklistData;
                console.log('[PrototypeLab.jsx] 🔄 재실행 체크리스트 데이터:', backendChecklistData);
                setChecklistData(prev => {
                  const updated = {
                    ...prev,
                    ...backendChecklistData
                  };
                  console.log('[PrototypeLab.jsx] 🔄 재실행 업데이트된 체크리스트 데이터:', updated);
                  return updated;
                });
              }
              
              // retryResponse 처리 (생성된 결과들 처리)
              if (retryResponse.generatedResults) {
                retryResponse.generatedResults.forEach(result => {
                  const resultMessage = { 
              ...
              console.error('[PrototypeLab.jsx] ❌ 재실행 실패:', error);
            }
          }, 1000);
        } else {
          // auto_retry가 없을 때만 로딩 메시지 제거
          console.log('[PrototypeLab.jsx] 🔄 auto_retry 없음 - 로딩 메시지 제거');
          setMessages(prev => prev.filter(msg => 
            !(msg.type === 'result' && msg.filePath === '')
          ));
        }
      ...
      .catch(error => {
        console.error('[PrototypeLab.jsx] ❌ sendChatMessage .catch() 진입');
        console.error('[PrototypeLab.jsx] ❌ API 호출 실패:', error);
  ...
  // 초기 파이프라인 호출 중복 방지 변수
  const initialRef = useRef(false);
  
  const handleInitialPipelineCall = async () => {
    // 이미 호출된 경우 중복 실행 방지
    if (initialRef.current) {
      console.log('[PrototypeLab.jsx] ⚠️ 초기 파이프라인 호출 이미 실행됨 - 중복 방지');
      return;
    }
    
    initialRef.current = true;
    console.log('[PrototypeLab.jsx] 🚀 초기 파이프라인 호출 시작');
    ...
    try {
      ...
      if (response && response.response) {
        ...
        console.log('[PrototypeLab.jsx] ✅ 초기 파이프라인 호출 완료');
      }
    } catch (error) {
      console.error('[PrototypeLab.jsx] ❌ 초기 파이프라인 호출 실패:', error);
    }
  };
    ...
    switch (messageType) {
      ...
      case 'result':
        ...
        if (!message.filePath || message.filePath.trim() === '') {
          // text 타입은 aiMessage와 동일한 형태로 표시
          if (message.resultType === 'text') {
            return (
              <div key={message.id} className="flex justify-start mb-6">
                <div className="bg-gray-800/90 backdrop-blur-md text-white rounded-2xl px-6 py-4 max-w-xs lg:max-w-2xl shadow-lg border border-gray-600/30">
              ...
              <div className="p-6 flex-1 overflow-y-auto">
                    ...
                    <div className="space-y-3 mt-3 pt-3 border-t border-gray-600/30">
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.car_size}
                          onChange={(e) => updateChecklistData('car_size', e.target.value)}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.car_boxes}
                          onChange={(e) => updateChecklistData('car_boxes', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 mt-3 pt-3 border-t border-gray-600/30">
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.wheelbase}
                          onChange={(e) => updateChecklistData('wheelbase', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="narrow/wide" 
                          value={checklistData.track_width}
                          onChange={(e) => updateChecklistData('track_width', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="front short, rear long" 
                          value={checklistData.overhang}
                          onChange={(e) => updateChecklistData('overhang', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="upright/low/aggressive" 
                          value={checklistData.stance}
                          onChange={(e) => updateChecklistData('stance', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="short/long" 
                          value={checklistData.dash_to_axle}
                          onChange={(e) => updateChecklistData('dash_to_axle', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.beltline}
                          onChange={(e) => updateChecklistData('beltline', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.greenhouse_size}
                          onChange={(e) => updateChecklistData('greenhouse_size', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                    </div>
                  ...
                  {selectedCategories.surfacing && (
                    <div className="space-y-2 mt-3 pt-3 border-t border-gray-600/30">
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.surfacing === "Clean"}
                          onChange={(e) => updateChecklistData('surfacing', e.target.checked ? "Clean" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.surfacing === "Taut"}
                          onChange={(e) => updateChecklistData('surfacing', e.target.checked ? "Taut" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.surfacing === "Soft"}
                          onChange={(e) => updateChecklistData('surfacing', e.target.checked ? "Soft" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.surfacing === "Chamfers"}
                          onChange={(e) => updateChecklistData('surfacing', e.target.checked ? "Chamfers" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.surfacing === "Bulges"}
                          onChange={(e) => updateChecklistData('surfacing', e.target.checked ? "Bulges" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.surfacing === "Strong shoulder line"}
                          onChange={(e) => updateChecklistData('surfacing', e.target.checked ? "Strong shoulder line" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                  ...
                  {selectedCategories.fascia && (
                    <div className="space-y-3 mt-3 pt-3 border-t border-gray-600/30">
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="parametric/mesh/slats" 
                          value={checklistData.grille_type}
                          onChange={(e) => updateChecklistData('grille_type', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="large/small" 
                          value={checklistData.air_intake}
                          onChange={(e) => updateChecklistData('air_intake', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="sporty/rugged" 
                          value={checklistData.bumper_style}
                          onChange={(e) => updateChecklistData('bumper_style', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="clamshell/sculpted" 
                          value={checklistData.hood_style}
                          onChange={(e) => updateChecklistData('hood_style', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="flush/pull type" 
                          value={checklistData.door_handles}
                          onChange={(e) => updateChecklistData('door_handles', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                    </div>
                  ...
                  {selectedCategories.lighting && (
                    <div className="space-y-3 mt-3 pt-3 border-t border-gray-600/30">
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="pixel DRL/strip DRL" 
                          value={checklistData.drl_type}
                          onChange={(e) => updateChecklistData('drl_type', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="LED/matrix/projector" 
                          value={checklistData.headlight_type}
                          onChange={(e) => updateChecklistData('headlight_type', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="full-width/vertical/horizontal" 
                          value={checklistData.taillight_type}
                          onChange={(e) => updateChecklistData('taillight_type', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="slim/parametric/pixelated" 
                          value={checklistData.light_shape}
                          onChange={(e) => updateChecklistData('light_shape', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 mt-3 pt-3 border-t border-gray-600/30">
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="자유 입력" 
                          value={checklistData.window_line}
                          onChange={(e) => updateChecklistData('window_line', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="chrome/black" 
                          value={checklistData.window_trim}
                          onChange={(e) => updateChecklistData('window_trim', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="body-color/gloss black" 
                          value={checklistData.side_mirror}
                          onChange={(e) => updateChecklistData('side_mirror', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="자유 입력" 
                          value={checklistData.pillar_treatment}
                          onChange={(e) => updateChecklistData('pillar_treatment', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                    </div>
                  ...
                  {selectedCategories.aero && (
                    <div className="space-y-2 mt-3 pt-3 border-t border-gray-600/30">
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.aero === "Splitter"}
                          onChange={(e) => updateChecklistData('aero', e.target.checked ? "Splitter" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.aero === "Vents"}
                          onChange={(e) => updateChecklistData('aero', e.target.checked ? "Vents" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.aero === "Roof rails"}
                          onChange={(e) => updateChecklistData('aero', e.target.checked ? "Roof rails" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                      ...
                      <label className="flex items-center space-x-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={checklistData.aero === "Roof spoiler"}
                          onChange={(e) => updateChecklistData('aero', e.target.checked ? "Roof spoiler" : "")}
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2" 
                        />
                    ...
                    <div className="space-y-3 mt-3 pt-3 border-t border-gray-600/30">
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.body_color}
                          onChange={(e) => updateChecklistData('body_color', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.wheel_design}
                          onChange={(e) => updateChecklistData('wheel_design', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          ...
                          value={checklistData.roof_color}
                          onChange={(e) => updateChecklistData('roof_color', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <input 
                          type="text" 
                          placeholder="chrome/gloss black/satin" 
                          value={checklistData.trim_accent}
                          onChange={(e) => updateChecklistData('trim_accent', e.target.value)}
                          className="w-48 bg-gray-700/80 border border-gray-600/50 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" 
                        />
                      </div>
                    </div>
                ...
                <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-600/30">
                  ...
                  <textarea 
                    ...
                    value={checklistData.free_input}
                    onChange={(e) => updateChecklistData('free_input', e.target.value)}
