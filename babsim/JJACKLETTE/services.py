...
MODEL_PATH = os.path.join(settings.BASE_DIR, 'models', 'kanana_model_files')
model = None
try:
    # model = KananaModel(model_path=MODEL_PATH)
...
def get_chatbot_response(user_query: str) -> str:
    ...
    try:
        ...
        # 임시 응답 로직 (테스트용)
        response = f"'{user_query}'에 대한 Kanana LLM의 답변입니다."
